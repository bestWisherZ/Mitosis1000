...
func NewConfig(nodeId uint32) *config.Config {
	...
	configData, err := loadConfigFromFile("bootnode.json")
	if err != nil {
	...
	return config.NewConfig("test", topo, 4, nodeId, accounts, configData.Bootnode)
}
...
func TestNewBFTProtocol(t *testing.T) {
	...
	for nodeId := uint32(1); nodeId <= 48; nodeId++ {
	...
	for nodeId := uint32(49); nodeId <= 528; nodeId++ {
